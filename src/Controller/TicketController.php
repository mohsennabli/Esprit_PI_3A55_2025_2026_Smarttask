...
class TicketController extends AbstractController
{
    ...
    public function index(TicketRepository $repo, Request $request): Response
    {
        ...
        if ($statut && $statut != '') {
            $tickets = $repo->findByStatut($statut);
        } elseif ($priorite && $priorite != '') {
            $tickets = $repo->findByPriorite($priorite);
        } elseif ($search) {
            $tickets = $repo->searchByKeyword($search);
        } else {
            $tickets = $repo->findAllOrderedByDate();
        }
